...
class Database():
    ...
    def giving_book(self,id_user,id_book,date_now,date_ret):
        self.cursor.execute("UPDATE library5 SET reader=%s, date_given_out=%s, date_return=%s WHERE id=%s",[id_user,date_now,date_ret,id_book])
    ...
    def users_on_name(self,all_name):
        ...
        if parse !=-1:
            ...
            self.cursor.execute("SELECT * FROM users WHERE name LIKE %s AND surname LIKE %s OR name LIKE %s AND surname LIKE %s", [search_in_table,search_in_sur,search_in_sur,search_in_table])
            return (self.cursor.fetchall())
        else:
    ...
    def search_fetchall(self, search_text, search_bool):
        ...
        self.cursor.execute("SELECT library5.id FROM library5 LEFT "
                            "JOIN books_authors ON library5.id = books_authors.book_id LEFT JOIN authors "
                            "ON authors.id = books_authors.author_id WHERE((False OR %s and author_name LIKE %s) "
                            "OR (%s and name LIKE %s)OR(%s and annotation LIKE %s))"
